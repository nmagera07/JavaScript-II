...
let allCaps = [];
    let allRunners = runners.map(runner => runner.first_name.toUpperCase())
    allCaps.push(allRunners)
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
    let newShirts = runners.filter(runner => runner.shirt_size === "L")
    largeShirts.push(newShirts)
...
// Problem 1
// Output the names of the runners as well as their email addresses
let fullInfo = [];
    runners.forEach(function (runner) {
    fullInfo.push(runner.first_name + " " + runner.last_name + ":" + runner.email)
})
console.log(fullInfo.sort());
// Problem 2
// Find how many donations were above 100
let donations = [];
    let newShirts = runners.filter(runner => runner.donation > 100)
